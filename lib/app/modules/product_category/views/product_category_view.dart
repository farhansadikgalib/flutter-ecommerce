    ...
    return Obx(() {
      return Scaffold(
        ...
        drawer: Drawer(
          child: Expanded(
            child: ListView(
              padding: REdgeInsets.only(left: 10,bottom: 50),
              children: [
                AppWidgets().gapH8(),
                 Visibility(
                   visible: controller.category.isNotEmpty,
                   child: Text('Categories', style: TextStyle(fontWeight: FontWeight
                       .bold)),
                 ),
                Obx(() => Column(
                  children: controller.category
                    ...
                    onChanged: (value) {
                      ...
                      printLog('Selected Category ID: ${category.id}');
                ...
                Obx(() => Column(
                  children: controller.brands
                    ...
                    onChanged: (value) {
                      ...
                      printLog('Selected Brand ID: ${brand.id}');
                ...
                Obx(() => Column(
                  children: controller.collections
                    ...
                    onChanged: (value) {
                      ...
                      printLog('Selected Collection ID: ${collection.id}');
                ...
                Obx(() => Column(
                  children: controller.deliveryType
                      .map((deliveryType) => RadioListTile(
                    title: Text(deliveryType.title ?? ''),
                    value: deliveryType.id,
                    groupValue: controller.deliveryType
                        .firstWhereOrNull((type) => type.isSelected == true)
                        ?.id,
                    onChanged: (value) {
                      controller.deliveryType.forEach((type) {
                        type.isSelected = type.id == value;
                      });
                      printLog('Selected Delivery Type ID: $value');
              ...
              RangeSlider(
                  ...
                  divisions: 20,
                  labels: RangeLabels(
                    controller.priceRange.start.round().toString(),
                    controller.priceRange.end.round().toString(),
                  ),
                  onChanged: (RangeValues values) {
                    controller.priceRange = values;
                    print('Price Range: ${values.start} - ${values.end}');
                  },
                ),
                      ...
                      return InkWell(
                        onTap: () {
                          Get.toNamed(
                            ...
                            arguments: {'product': ProductCollection(
                              id: product.id,
                              title: product.title,
                              slug: product.slug,
                              image: product.image,
